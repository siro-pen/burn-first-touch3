...
import { 
  ...
  Info, Zap, Copy, Check, RotateCcw, ShieldAlert
} from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const generateAdvice = async () => {
    ...
    try {
      ...
      setAiAdvice(advice || "解析結果を取得できませんでした。");
    } catch (e) {
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row pb-20 md:pb-0">
      <nav className="hidden md:flex w-64 bg-white border-r border-slate-200 flex-col p-6 sticky top-0 h-screen">
        ...
        <div className="space-y-2">
          <button onClick={() => setActiveTab('calc')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-all ${activeTab === 'calc' ? 'bg-blue-600 text-white shadow-lg shadow-blue-200' : 'text-slate-400 hover:bg-slate-50'}`}>
            <Calculator className="w-5 h-5" />評価・計算
          </button>
          <button onClick={() => setActiveTab('ai')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-all ${activeTab === 'ai' ? 'bg-blue-600 text-white shadow-lg shadow-blue-200' : 'text-slate-400 hover:bg-slate-50'}`}>
            <MessageSquare className="w-5 h-5" />AI分析
          </button>
          <button onClick={() => setActiveTab('info')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-all ${activeTab === 'info' ? 'bg-blue-600 text-white shadow-lg shadow-blue-200' : 'text-slate-400 hover:bg-slate-50'}`}>
          ...
          <div className="space-y-6">
            <header className="flex justify-between items-center px-1">
              ...
              {aiAdvice && (
                <button 
                  onClick={copySummary} 
                  className={`p-2 transition-colors ${isCopied ? 'text-green-500' : 'text-blue-600'}`}
                >
